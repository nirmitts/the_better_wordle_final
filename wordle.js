...
var width = 5; //no. of letters in a word
var currentRow = 0; //current guest (attempt no.)
var currentCol = 0; //current letter for that attempt
var gameOver = false;
...
const staggerMs = 300;
const flipMs = 500;
const totalAnimTime = (width - 1) * staggerMs + flipMs + 100; // small buffer
let animating = false;
...
function initialize() {
    ...
    for (let i = 0; i < keyboard.length; i++) {
        ...
        for (let j = 0; j < currRow.length; j++) {
            ...
            else if ("A" <= key && key <= "Z") { //only for letters
                keyTile.id = "Key" + key; //eg. "Key" + "A" => KeyA, KeyB, KeyC
            }
    ...
    document.addEventListener("keyup", (e) => {
        ...
        if (tag === "INPUT" || tag === "TEXTAREA") {
            return; // user is typing in a form field — don't feed it into the game
        }
        processInput(e);
    });
    ...
    document.getElementById("howToPlayModal").addEventListener("click", (e) => {
        if (e.target.id === "howToPlayModal") closeModal("howToPlayModal");
    });

    document.getElementById("closeResultBtn").addEventListener("click", hideResultModal);

    document.getElementById("resultModal").addEventListener("click", (e) => {
        if (e.target.id === "resultModal") hideResultModal();
    });
    ...
    // loginBtn: opens profile modal if logged in, else opens auth modal
...
// ── Modal helpers with timer pause/resume ──────────────────────────────────
let openModalCount = 0; // track how many modals are open

function openModal(id) {
    document.getElementById(id).classList.add("show");
    openModalCount++;
    if (openModalCount === 1 && !gameOver) pauseTimer();
}

function closeModal(id) {
    document.getElementById(id).classList.remove("show");
    openModalCount = Math.max(0, openModalCount - 1);
    if (openModalCount === 0 && !gameOver) resumeTimer();
}
...
function resumeTimer() {
    if (timerInterval) return; // already running (shouldn't happen, but safe)
...
function openProfileModal() {
    if (!userStats) return;
    const winPct = userStats.games_played > 0
        ? Math.round((userStats.games_won / userStats.games_played) * 100)
        : 0;
    document.getElementById("profileAvatar").textContent = userStats.username.charAt(0).toUpperCase();
    document.getElementById("profileUsername").textContent = userStats.username;
    document.getElementById("profileStreak").textContent  = userStats.current_streak;
    document.getElementById("profileBest").textContent    = userStats.best_streak;
    document.getElementById("profilePlayed").textContent  = userStats.games_played;
    document.getElementById("profileWinPct").textContent  = winPct + "%";
...
async function handleAuthSubmit() {
    ...
    const mode = document.getElementById("authBox").dataset.mode;
...
function processKey() {
    let e = { code: this.id }; //this.id is the id of the key pressed
    processInput(e);
}
...
function processInput(e) {
    ...
    //alert(e.code); to check the key pressed

    if ("KeyA" <= e.code && e.code <= "KeyZ") { //not A but KeyA
        while (currentCol < width && isHinted(currentRow, currentCol)) {
            currentCol += 1;
        }
        
        if (currentCol < width) {              //width = 5, so currentCol < 5
                let currTile = document.getElementById(currentRow.toString() + "-" + currentCol.toString());
                if (currTile.innerText == "") {
                    currTile.innerText = e.code[3];     //we want only letter i.e. KeyA => A, KeyB => B
                    currentCol += 1;
                }
            }
        }
        ...
        while (currentCol > 0) {
            currentCol -= 1;
            if (isHinted(currentRow, currentCol)) {
                continue; // locked tile, skip past it and keep going back
            }
            let currTile = document.getElementById(currentRow.toString() + "-" + currentCol.toString());
            currTile.innerText = "";
            break;
        ...
        if (currentCol == width) { //checks that user has entered 5 letters before pressing enter
            update();
        }
        else {
...
//obsolete as it causes time conflict
//      if (!gameOver && !animating && currentRow == height) {
//         animating = true;
//         setTimeout(() => {
//             gameOver = true;
//             stopTimer();
//             showMessage("Better luck next time..." + "\nAnswer: " + word, { persistent: true });
//             animating = false;
//         }, totalAnimTime);
//     }
// }
...
/*function update() {
    ...
    if (guessedWords.includes(guess)) { //for not allowing duplicate words to be guessed
    ...
    let correct = 0; //no. of correct letters in correct position
    let letterCount = {}; //to count the no. of occurences of letters in the word eg. TABBY = {T:1, A:1, B:2, Y:1}
    ...
    //first iteration to check for correct letters in correct position
    for (let c = 0; c < width; c++) {
        let currTile = document.getElementById(currentRow.toString() + "-" + c.toString());
        let letter = currTile.innerText;
        ...
        if (word[c] == letter) {
            ...
            correct += 1;
            letterCount[letter] -= 1; //decrease the count of that letter in the word
        }
    }

    if (correct == width) { //If user guessed  word correctly before all attempts, then game is over
        gameOver = true;
        showMessage("You guessed it! 🎉", { persistent: true });
        stopTimer(); //stop the timer if user guessed the word correctly
    }
    ...
    for (let c = 0; c < width; c++) {
        ...
        if (!currTile.classList.contains("correct")) { //if not already marked as correct
            //Is it in the word?
            if (word.includes(letter) && letterCount[letter] > 0) {
                ...
                if (!keyTile.classList.contains("correct")){  //if not already marked as correct
    ...
    currentRow += 1; //start new row
    currentCol = 0; //start from first column of new row
}
*/
...
//     // determine each tile's result first, without touching the DOM colors yet
//     let results = [];
//     for (let c = 0; c < width; c++) {
//         let currTile = document.getElementById(currentRow.toString() + "-" + c.toString());
//         let letter = currTile.innerText;
...
//             letterCount[letter] -= 1;
//         } else {
//             results.push(null); // fill in second pass
//         }
//     }
//     for (let c = 0; c < width; c++) {
//         if (results[c] !== "correct") {
//             let currTile = document.getElementById(currentRow.toString() + "-" + c.toString());
//             let letter = currTile.innerText;
//             if (word.includes(letter) && letterCount[letter] > 0) {
//                 results[c] = "present";
...
function update() {
    ...
    guess = guess.toLowerCase();
    if (!guessSet.has(guess)) { //Switching from wordList.includes(guess) — an O(n) array scan — to guessSet.has(guess) — an O(1) Set lookup — is also a nice performance win now that the guess list is much bigger.)
        showMessage("Not in word list!"); // instant
        return;
    }
    if (guessedWords.includes(guess)) {
        showMessage("Already guessed that word!"); // instant
        return;
    }
    guessedWords.push(guess);
    ...
    for (let c = 0; c < width; c++) {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                currTile.classList.add(result);
            }, 250);
            setTimeout(() => {
                applyKeyColor(letter, result);
            }, 500);
        }, c * staggerMs);
    }

    // obsolete causes time conflict
    // if (correct == width) {
    //     setTimeout(() => {
    //         gameOver = true;
    //         stopTimer();
    //         showMessage("Congratulations, you guessed it! 🎉", { persistent: true });
    //         launchConfetti();
    //         animating = false;
    //     }, totalAnimTime);
    // } else {
    ...
    // currentCol = 0;
        ...
        if (correct == width) {
            const winRow = currentRow;
            setTimeout(async () => {
                if (gameOver) return;
                gameOver = true;
                stopTimer();
                showMessage(winMessages[winRow], { persistent: true });
                launchConfetti();
                animating = false;
                await recordGameResult(true);
                showResultModal("YOU WON!", winMessages[winRow], true);
            }, totalAnimTime);
            ...
            setTimeout(async () => {
                ...
                showResultModal("TRY AGAIN", "You've used all your guesses.", true);
            }, totalAnimTime);
        } else {
            setTimeout(() => {
                animating = false;
    ...
    // now animate + apply colors + keyboard updates with stagger
    // for (let c = 0; c < width; c++) {
    //     let currTile = document.getElementById(currentRow.toString() + "-" + c.toString());
    //     let letter = currTile.innerText;
    //     let result = results[c];
    ...
    //                 }
    //             }
    //         }, 250); // matches the 50% point of the 0.5s flip animation
    //     }, c * 300); // stagger each tile by 300ms
    // }
}
...
function startTimer() {
  ...
  timerInterval = setInterval(() => {
    ...
    if (timeLeft <= 60) {
      timerEl.classList.add('danger'); //overrides warning if timeLeft <= 60
    } else if (timeLeft <= 180) {
...
async function handleTimeUp() {
  ...
  await recordGameResult(false);
  showResultModal("TIME'S UP!", "You ran out of time.", true);
}
...
function useHint() {
    ...
    if (currentCol >= width) {
        showMessage("Row is already full!");
        return;
    }

    hintsUsed += 1;

    let letter = word[currentCol];
    let currTile = document.getElementById(currentRow.toString() + "-" + currentCol.toString());
    let col = currentCol;
    ...
    currTile.classList.add("reveal");
    setTimeout(() => {
        currTile.classList.add("hinted");
    }, 250);

    setTimeout(() => {
        let keyTile = document.getElementById("Key" + letter);
        if (keyTile && !keyTile.classList.contains("correct")) {
            keyTile.classList.add("hinted");
        }
    }, 500);
}
...
function isHinted(row, col) {
    ...
    return tile.classList.contains("hinted");
}
...
function initTheme() {
    ...
    document.getElementById("themeToggle").innerText = saved === "dark" ? "☀️" : "🌙";
...
function showMessage(text, options = {}) { //for showing messages for 2.5 seconds if not persistent
    const { duration = 2500, persistent = false } = options;
...
function applyKeyColor(letter, result) {
    let keyTile = document.getElementById("Key" + letter);
    if (keyTile.classList.contains("correct")|| keyTile.classList.contains("hinted")) {
        return; // correct is permanent, never downgrade
    }
    if (result == "correct") {
...
function launchConfetti() {
    ...
    const colors = ["#6aaa64", "#c9b458", "#3a86ff", "#f4a261", "#e63946", "#95d5b2"];
    let pieces = [];
    ...
    for (let i = 0; i < pieceCount; i++) {
        pieces.push({
            ...
            speedY: 3 + Math.random() * 4,      // slightly faster so it clears in reasonable time
    ...
    function draw() {
        ...
        // remove pieces once they've fully fallen past the bottom of the screen
...
//background particles in wallpaper
function createParticles() {
    const container = document.getElementById("particles");
    const greens = ["#1b4332", "#2d6a4f", "#40916c", "#52b788", "#74c69d", "#95d5b2"];
    const count = 30;

    // estimate the width of the central game content (board/keyboard/heading)
    const contentWidth = 420; // roughly matches your #board + keyboard width
    ...
    for (let i = 0; i < count; i++) {
        ...
        dot.style.background = greens[Math.floor(Math.random() * greens.length)];

        // pick a position in the LEFT gutter or RIGHT gutter only, never the center
        let x;
        if (gutter > 40) { // only bother if there's actually meaningful empty space
            const onLeft = Math.random() < 0.5;
            if (onLeft) {
                x = Math.random() * (gutter - size);
            } else {
                x = viewportWidth - gutter + Math.random() * (gutter - size);
            }
        } else {
            // narrow/mobile screens: skip placing particles, content fills the width
            x = -100; // push off-screen so it's effectively hidden
        }
        dot.style.left = x + "px";

        dot.style.bottom = "-5vh";
        dot.style.animationDuration = (15 + Math.random() * 20) + "s";
        dot.style.animationDelay = (Math.random() * 20) + "s";

        // give each particle a unique slight horizontal drift for a natural, non-uniform float
        const drift = -30 + Math.random() * 60; // between -30px and +30px
...
let resizeTimeout; //This regenerates the particles (debounced to avoid firing constantly while dragging the window edge) so they stay correctly confined to the gutters whenever the screen size changes.
...
function showResultModal(title, message, showAnswer) {
    document.getElementById("resultTitle").innerText = title;
    document.getElementById("resultMessage").innerText = message;
    document.getElementById("resultAnswer").innerText = showAnswer ? word : "";

    const streakEl = document.getElementById("streakInfo");
    const loginPromptEl = document.getElementById("loginPrompt");

    if (currentUser && userStats) {
        document.getElementById("streakInfoCurrent").innerText = userStats.current_streak;
        document.getElementById("streakInfoBest").innerText = userStats.best_streak;
...
// =============================================
// SUPABASE AUTH — REAL APPLICATION LOGIN SYSTEM
// =============================================

const supabaseClient = window.supabase.createClient(
  "https://xzfrxbxrwqflodebsxvd.supabase.co",
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inh6ZnJ4Ynhyd3FmbG9kZWJzeHZkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODQxMTI4NjcsImV4cCI6MjA5OTY4ODg2N30.Ric_6H8XlJ_kZ0uXddzldr5dZ_bD1LlSPtQ8RxlIUjY"
);

// Convert a plain username to a fake email (Supabase auth requires email)
function usernameToEmail(username) {
    return username.toLowerCase().trim() + "@betterwordle.local";
}

// ── Auth State Change Listener ──────────────────────────────────────────────
// Runs on page load (restores session) and whenever auth state changes.
supabaseClient.auth.onAuthStateChange(async (event, session) => {
    if (session && session.user) {
        // User is signed in (either from a fresh login or a restored session)
        ...
        document.getElementById("streakWrapper").style.display = "none";
    }
});
...
async function loadUserProfile(user) {
    ...
    if (data) {
        userStats = data;
        document.getElementById("loginBtn").innerText = "PROFILE | " + data.username;
        updateStreakDisplay();
    }
}
...
async function logIn(username, password) {
    ...
    if (error) {
        // Distinguish between "user doesn't exist" vs "wrong password"
    ...
    // Success: onAuthStateChange will call loadUserProfile automatically
    location.reload();
}
...
async function register(username, password, confirmPassword) {
    ...
    // 3. Insert profile row linked to auth user
    ...
    if (profileError) {
        showAuthError("Account created but profile setup failed. Try logging in.");
        return false;
    }
    ...
    if (signInError) {
        // Account created, but couldn't auto-login — show success and prompt manual login
        showAuthSuccess("Account created! Please log in.");
        switchAuthMode("login");
        return true;
    }

    // onAuthStateChange will load the profile automatically
    location.reload();
}
...
// Kept for backwards compatibility; internal code still calls this
...
function updateStreakDisplay() {
    if (!userStats) return;
    document.getElementById("streakCount").innerText = userStats.current_streak;
    document.getElementById("tooltipCurrent").innerText = userStats.current_streak;
    document.getElementById("tooltipBest").innerText = userStats.best_streak;
    document.getElementById("streakWrapper").style.display = "inline-block";
}

async function recordGameResult(won) {
    if (!currentUser || !userStats) return;

    let newStreak = won ? userStats.current_streak + 1 : 0;
    let newBest = Math.max(userStats.best_streak, newStreak);
    ...
    const { data, error } = await supabaseClient
        ...
        .update({
            ...
            games_won: userStats.games_won + (won ? 1 : 0)
        })
...
function switchAuthMode(mode) {
    ...
    if (mode === "login") {
        ...
        confirmField.classList.add("auth-hidden");
        tabLogin.classList.add("active");
        tabRegister.classList.remove("active");
    } else {
        title.textContent = "Create Account";
        submitBtn.textContent = "Create Account";
        confirmField.classList.remove("auth-hidden");
